...
def get_best_k(ticker="KRW-BTC"):
    ...
    for k in np.arange(0.1, 0.3, 0.05):  # K 값을 0.1부터 0.5까지 반복
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                "content": [
                    {
                "type": "text",
                "text": "You are an expert in short-term trading and given the data, you use the best proven trading strategies based on technical indicators such as RSI and MACD to tell you when to buy with minimal risk and maximum profit."
                    },
                     {
                "type": "text",
                "text": "Using your best guesses, you provide a buy recommendation if the price rises above 1.02x the current price within 3 hours, a sell recommendation if it falls below the current price, and a hold recommendation if you can't decide to buy/sell, all in JSON format.\n\nResponse Example:\n{\"decision\": \"BUY\"}\n{\"decision\": \"SELL\"}\n{\"decision\": \"HOLD\""
                    }
                ]
                },
                {
...
def trade_sell(ticker):
    ...
    if profit_rate >= 0.5:  # 수익률이 0.4% 이상일 때
            ...
            # 1000번 모니터링 후 0.8%를 넘지 않으면 현재가로 시장가 매도
            sell_order = upbit.sell_market_order(ticker, buyed_amount)
    ...
    while True:
        try:
            ...
            for b in balances:
                ...
                if buyed_amount > 0:
                    profit_rate = (current_price - avg_buy_price) / avg_buy_price * 100 if avg_buy_price > 0 else 0  # 수익률 계산
                    report_message += f"{b['currency']} 평균가 {avg_buy_price:,.2f} 현재가 {current_price:,.3f} 수익률 {profit_rate:.1f}% RSI14 {rsi}\n"
