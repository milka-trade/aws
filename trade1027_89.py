...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            # df_week = pyupbit.get_ohlcv(t, interval="week", count=3)  
            # if df_day is None or df_day.empty or 'high' not in df_day or 'low' not in df_day or 'open' not in df_day:
            #     continue  
            ...
            if len(df_day) >= 3:
                ...
                # day_volume_1 = df_day['volume'].iloc[-1]      #일봉 9시 기준 당일 거래량 
            ...
            # week_close_price_1 = df_week['close'].iloc[-1]  #9시 기준 당일 시가 
            
            day_open_price_1 = df_day['open'].iloc[-1]  #9시 기준 당일 시가 
            # day_close_price_1 = df_day['close'].iloc[-1]  #9시 기준 당일 종가 
            # day_close_price_2 = df_day['close'].iloc[-2]  #9시 기준 당일 종가 
            ...
            if target_price * 0.98 <= current_price < target_price * 1.000 :
                    ...
                    for i in range(buy_attempts):
                        try:
                            ...
                            send_discord_message(f"매수 성공: {ticker}, 목표가 98% {target_price*0.98:,.2f} < 목표가 {target_price:,.2f} < 현재가 {current_price:,.2f} / (시도 {attempt + 1}/{max_retries})")
                            return buy_order
                        except Exception as e:
