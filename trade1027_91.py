...
def get_atr(ticker, period=14):
    ...
    high_close = abs(df_atr_day['high'] - df_atr_day['close'].shift())
    low_close = abs(df_atr_day['low'] - df_atr_day['close'].shift())
...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            # print(f"cond2: {t} / 분봉 평균 7시간 거래량:{avg_min60_value:,.0f} =< 60봉 거래량:{min60_value:,.0f}")
                # if avg_week_value * 1.2 <= day_value_2 :  # Volume surge
                #     print(f"cond2: {t} / 전일 거래량 평균 거래량 대비 20% 상승")
                ...
                # if threshold_value < atr :  # Volatility check
                    # print(f"cond3: {t} / 임계값:{threshold_value:,.2f} < 평균진폭:{atr:,.2f}")
                        ...
                        # if 30 < rsi < 60: # RSI in a favorable range
