...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            df_day = pyupbit.get_ohlcv(t, interval="day", count=20)   #일봉 20개 조회
            ...
            day_ma20=df_day['close'].rolling(window=20).mean().iloc[-1] if df is not None and not df.empty else 0

            if day_volume >= 100_000_000:  #1.(상승지표)전일 거래량 100백만 이상
                if day_open_price <= cur_price and day_open_price * 1.06 > cur_price : #2.(상한설정)일봉기준 시가 대비 6% 이내 상승 
                        if day_ma20 <= cur_price:  #(상승지표)일봉기준 현재가가 20이평 이상 상승
...
def trade_buy(ticker, k):
    ...
    if buyed_amount == 0 and ticker.split("-")[1] not in ["BTC", "ETH"] and krw >= 5000 :  # 매수 조건 확인
        if target_price <= current_price and current_price < target_price*1.03 :  #현재가가 목표가의 3% 이내인 경우
...
def trade_sell(ticker):
    ...
    if buyed_amount > 0 :  # 보유잔고가 0 이상이면
        if profit_rate > 3:  # 3% 이상 수익률일 때 AI의 판단을 구함
            ai_decision = get_ai_decision(ticker)  # AI의 판단을 구함
            send_discord_message(f"{ticker} AI:{ai_decision}")
            if ai_decision == 'SELL' or profit_rate > 5:  # AI의 판단이 sell or 수익률이 5%를 넘는 경우 매도
                sell_order = upbit.sell_market_order(ticker, buyed_amount)  # 시장가로 매도
                send_discord_message(f"매도: {ticker}, 현재가 {current_price} 수익률 {profit_rate:.2f}% (수익률 3% 초과 또는 AI 판단 sell)")
                return sell_order
