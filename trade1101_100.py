...
def get_best_k(ticker="KRW-BTC"):
    ...
    for k in np.arange(0.05, 0.15, 0.05):  # K 값을 0.05부터 0.15까지 반복
...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            # stoch_rsi = get_stoch_rsi(t, 14, 14)   #스토캐스틱 RSI 계산
...
def trade_buy(ticker, k):
    ...
    # New Indicators and Patterns #2
    ha_df = calculate_heikin_ashi(df)   #하이킨 아시 캔들 계산
    ema200 = get_ema(ticker, 200)    #200봉 가중이동평균 계산
    stoch_rsi = get_stoch_rsi(ticker, 14, 14)   #스토캐스틱 RSI 계산
    ...
    while True:
        try:
            ...
            for b in balances:
                ...
                stoch_rsi = get_stoch_rsi(ticker, 14, 14)

                if buyed_amount > 0:
                    profit_rate = (current_price - avg_buy_price) / avg_buy_price * 100 if avg_buy_price > 0 else 0  # 수익률 계산
                    report_message += f"{b['currency']} 수익률 {profit_rate:.1f}% Stoch_RSI14 {stoch_rsi}\n"
