...
def load_ohlcv(ticker):
    ...
    if ticker not in df_tickers:   # 티커가 캐시에 없으면 데이터 가져오기     
        try:
            df_tickers[ticker] = pyupbit.get_ohlcv(ticker, interval="minute60", count=20) 
...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            df_day = pyupbit.get_ohlcv(t, interval="day", count=5)   #3일봉 조회
            ...
            day_open_price = df_day['open'].iloc[0]   # 당일 시가 조회
            day_volume = df_day['volume'].iloc[-1]      # 현재봉 거래량
            df_open=df['close'].iloc[0]
            ma20=get_ma20(t)

            if day_volume >= 100_000_000:  #1.(상승지표)전일 거래량 100백만 이상
                if day_open_price <= cur_price and day_open_price * 1.6 > cur_price : #2.(상한설정)일봉기준 시가 대비 6% 이내 상승 
                    # if df_open <= cur_price <= df_open*1.02 :  #(상한 설정)현재가가 현재봉의 0~2% 이내
                        if ma20 <= cur_price:  #(상승지표)현재가가 20이평 이상 상승
...
def trade_buy(ticker, k):
    ...
    if buyed_amount == 0 and ticker.split("-")[1] not in ["BTC", "ETH"] and krw >= 5000 :  # 매수 조건 확인
        if target_price*0.99 <= current_price and current_price < target_price*1.03 :  #현재가가 목표가의 99%~103% 이내
...
def trade_sell(ticker):
    ...
    """매도제한시간 체크 : EC2 변환"""
    sell_start = current_time.replace(hour=23, minute=59, second=0, microsecond=0)    #EC2
    sell_end = current_time.replace(hour=0, minute=3, second=0, microsecond=0)      #EC2
    # sell_start = current_time.replace(hour=8, minute=59, second=00, microsecond=0)       #VC
    # sell_end = current_time.replace(hour=9, minute=3, second=50, microsecond=0)         #VC
    ...
    if sell_start <= current_time <= sell_end:      # 매도 제한시간이면
    ...
    if buyed_amount > 0 :  # 보유잔고가 0 이상이면
        if profit_rate > 0.65:  # 0.65% 이상 수익률일 때 AI의 판단을 구함
            ...
            if ai_decision == 'SELL' or profit_rate > 1.2:  # AI의 판단이 not buy or 수익률이 1.2%를 넘는 경우 매도
                sell_order = upbit.sell_market_order(ticker, buyed_amount)  # 시장가로 매도
                send_discord_message(f"매도: {ticker}, 현재가 {current_price} 수익률 {profit_rate:.2f}% (수익률 1.2% 초과 또는 AI 판단 sell)")
                return sell_order
...
now = datetime.now().strftime('%m/%d %H:%M:%S')  # 시작시간 기록
print(f'{now} trading start')
...
while True:
    try:
        current_time = datetime.now()
        """매수제한시간 체크 : EC2 변환"""
        # restricted_start = current_time.replace(hour=8, minute=0, second=0, microsecond=0)     # 08:00  #vC2
        # restricted_end = current_time.replace(hour=10, minute=0, second=0, microsecond=0)      # 10:20   #vC2
        restricted_start = current_time.replace(hour=23, minute=0, second=0, microsecond=0)  # 23:00  #EC2
        restricted_end = current_time.replace(hour=1, minute=0, second=0, microsecond=0)    # 10:20    #EC2
