...
def load_ohlcv(ticker):
    ...
    if ticker not in df_tickers:   # 티커가 캐시에 없으면 데이터 가져오기     
        try:
            df_tickers[ticker] = pyupbit.get_ohlcv(ticker, interval="minute60", count=25)
            # print(df_tickers)
...
def trade_buy(ticker, k):
    ...
    if buyed_amount == 0 and ticker.split("-")[1] not in ["BTC", "ETH"] and krw >= 5000 :  # 매수 조건 확인
            ...
            if target_price <= current_price and current_price < target_price*1.1 :  #현재가가 목표가 이상이면서 목표가의 10% 이내
                ai_decision = get_ai_decision(ticker)  
                print(ai_decision)
                if ai_decision != 'SELL' :  # AI의 판단이 NOT SELL이면
                    try:
                        buy_order = upbit.buy_market_order(ticker, 110_000)
                        print(f"매수: {ticker}, 현재가: {current_price}")
                        send_discord_message(f"매수 {ticker}, 목표가: {target_price} 현재가: {current_price} 이평5: {ma5:.2f}, 이평15: {ma15:.2f}, AI: {ai_decision}")
                        # time.sleep(5)  # API 호출 제한을 위한 대기
...
def trade_sell(ticker):
    ...
    # 매도 시간 체크
    # sell_start = current_time.replace(hour=23, minute=59, second=00, microsecond=0)
    # sell_end = current_time.replace(hour=23, minute=59, second=50, microsecond=0)
    sell_start = current_time.replace(hour=8, minute=59, second=00, microsecond=0)
    sell_end = current_time.replace(hour=8, minute=59, second=50, microsecond=0)
    ...
    if sell_start <= current_time <= sell_end:
        ...
        for b in balances:
            if b['currency'] not in ["KRW", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:  # 보유 잔고가 있는 경우
                ticker = f"KRW-{b['currency']}"  # 티커 형식 맞추기
                sell_order = upbit.sell_market_order(ticker, buyed_amount)  # 시장가로 전량 매도
                send_discord_message(f"전량 매도: {ticker}, 수익률: {profit_rate:.2f}%")
                return sell_order
    
    if buyed_amount > 5000 :  # 매수 금액이 5000 이상인 경우에만 매도 조건 체크
        if currency not in ["BTC", "ETH"]:
        
            if profit_rate > 0.6:  # 0.6% 이상 수익률일 때 AI의 판단을 구함
                ai_decision = get_ai_decision(ticker)  # AI의 판단을 구함
            
                if ai_decision != 'BUY' or profit_rate > 2:  # AI의 판단이 BUY가 아니거나 수익률이 2%를 넘는 경우 매도
                    ...
                    send_discord_message(f"매도: {ticker}, 수익률: {profit_rate:.2f}% (수익률 2% 초과 또는 AI 판단 NOT BUY)")
                    return sell_order
...
while True:
    try:
        current_time = datetime.now()
        restricted_start = current_time.replace(hour=8, minute=0, second=0, microsecond=0)  # 08:00
        restricted_end = current_time.replace(hour=9, minute=30, second=0, microsecond=0)  # 09:30
        # restricted_start = current_time.replace(hour=23, minute=0, second=0, microsecond=0)  # 23:00
        # restricted_end = current_time.replace(hour=0, minute=30, second=0, microsecond=0)  # 00:30
        ...
        if krw_balance < 100000:
            ...
            print(best_ticker, interest, best_k)
            # 매수 제한 시간 체크
            if not (restricted_start <= current_time <= restricted_end):  # 제한 시간 외에 매수

                if best_ticker:  # 최고의 코인이 존재할 경우 매수시도
                    # print("매수시도")
                    result = trade_buy(best_ticker, best_k)  # 매수 실행
                    time.sleep(10)  # 매수 후 잠시 대기
                    balances = upbit.get_balances()  # 모든 보유 코인 조회
                    for b in balances:
                        if b['currency'] not in ["KRW", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:  # 보유 잔고가 있는 경우
                            ticker = f"KRW-{b['currency']}"  # 티커 형식 맞추기
