...
def load_ohlcv(ticker):
    ...
    if ticker not in df_tickers:   # 티커가 캐시에 없으면 데이터 가져오기     
        try:
            df_tickers[ticker] = pyupbit.get_ohlcv(ticker, interval="minute5", count=200) 
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                "content": [
                    {
                "type": "text",
                "text": "You are an expert in short-term cryptocurrency trading and based on the data given to you, make the best inference you can and give a buy recommendation if it rises more than 1.03x from the current price within 5 hours, a sell recommendation if it falls below the current price, or a hold recommendation if you can't decide to buy or sell in JSON format.\n\nResponse Example:\n{\"decision\": \"BUY\"}\n{\"decision\": \"SELL\"}\n{\"decision\": \"HOLD\"}"
                    },
