...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            last_wma200 = get_wma(t, 200).iloc[-1]    #200봉 가중이동평균 계산
            pre_wma200 = get_wma(t, 200).iloc[-2]
                    ...
                    if last_ha_close > last_ha_open :
                        ...
                        if pre_ha_close < last_ha_close :
                            ...
                            if previous_stoch_rsi <= 0.20 and last_stoch_rsi > 0.20:
                                print(f"cond2-4: {t} Pre_s_RSI:{previous_stoch_rsi:,.2f} <= 0.2 < Last_s_RSI:{last_stoch_rsi:,.2f}")
                        # if previous_stoch_rsi <= 0.20 and last_stoch_rsi > 0.20:
