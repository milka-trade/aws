...
# Example dynamic threshold function for ATR
...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            rsi = get_rsi(t, period=14)
            ma20 = get_sma(t, 20)
            ma5 = get_sma(t, 5)
            avg_weekly_volume = df_day['value'].rolling(window=7).mean().iloc[-1]
            atr = get_atr(t)

            if day_value_1 > 25_000_000_000 or day_value_2 > 25_000_000_000 :    
                # print(f"cond1: {last_day_date} / {t} / 당일 거래량 > 25십억:{day_value_1:,.0f} / 전일 거래량 > 25십억:{day_value_2:,.0f}")
                              
                if day_open_price_1 < cur_price < day_open_price_1*1.05:
                    print(f"cond2: {t} / 일봉 시가: {day_open_price_1:,.2f} < price: {cur_price:,.2f} < 일봉 5%:{day_open_price_1*1.05:,.2f}")
                        
                    if ma20 < cur_price < df_open_1*1.03 :    
                        print(f"cond3: {t} / MA20: {ma20:,.2f} < price:{cur_price:,.2f} < 분봉시가 3%:{df_open_1*1.03:,.2f}")

                        if 30 < rsi < 70: # RSI in a favorable range
                            print(f"cond4: {t} / 30 < rsi:{rsi:,.2f} < 70")

                            if ma20 < ma5 :  # Short-term momentum
                                print(f"cond5: {t} / ma20:{ma20:,.2f} < ma5:{ma5:,.2f}")

                                if day_value_1 > avg_weekly_volume * 1.5:  # Volume surge
                                    print(f"cond6: {t} / 주 평균 거래량:{avg_weekly_volume*1.5:,.0f} < 일봉거래량:{day_value_1:,.0f}")

                                    if atr > threshold_value:  # Volatility check
                                        print(f"cond7: {t} / 평균진폭:{atr:,.2f} > 임계값:{threshold_value:,.2f}")
...
def trade_sell(ticker):
    ...
    if sell_start <= selltime <= sell_end:      # 매도 제한시간이면
        ...
        while attempts < max_selltime_attempts:
            ai_decision = get_ai_decision(ticker)  
            if ai_decision == "SELL" :
                sell_order = upbit.sell_market_order(ticker, buyed_amount)  # 시장가로 전량 매도
                send_discord_message(f"전량 매도: {ticker}, 현재가 {current_price} 수익률 {profit_rate:.2f}% AI {ai_decision}")
                return sell_order
            else:
                send_discord_message(f"{ticker} AI NOT SELL")
