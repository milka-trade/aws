    ...
    for t in tickers:
        ...
        try:
            ...
            #     print(f"1.거래량 {t} 60봉 1,500백만:{min60_value_1:,.0f}")
                ...
                if day_close_price_1 <= cur_price <= day_close_price_1*1.05:
                    if cur_price <= week_close_price_1*1.20:                            
                        # print(f"2.일봉 0~5%이내/주봉 20% 이내 {t}")
                                    
                        if ma20 < cur_price < df_close_1*1.03 :    #4.현재가가 60분봉 20일 이동평균 이상이고 시가보다 큼 (상승지표)
...
def trade_buy(ticker, k):
    ...
    max_retries = 40  # Maximum attempts to check if price is within range
    ...
    while True:
        try:
            ...
            for b in balances:
                ...
                if buyed_amount > 0:
                    profit_rate = (current_price - avg_buy_price) / avg_buy_price * 100 if avg_buy_price > 0 else 0  # 수익률 계산
                    report_message += f"{b['currency']} 평균가 {avg_buy_price:,.2f} 현재가 {current_price:,.3f} 수익률 {profit_rate:.1f}% rsi {latest_rsi} \n"
