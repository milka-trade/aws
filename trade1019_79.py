    ...
    for t in tickers:
        ...
        try:
            ...
            if len(df_day) >= 3:
                ...
                send_discord_message(f"Insufficient data for ticker: {t}")
                continue
            ...
            if day_value_1 >= 25_000_000_000: 
                ...
                if day_open_price < cur_price and day_open_price*1.1 > cur_price :                            
                    # if df_close_3 > df_close_2 :                           #3.2봉전 종가보다 1봉전 종가보다 작음 (음봉)
                        if ma20 < cur_price and df_open_1 < cur_price :    #4.현재가가 60분봉 20일 이동평균 이상이고 시가보다 큼 (상승지표)
                            # print(f"{t} / price:{cur_price}/ 시가:{df_open_1} / ma20:{ma20:,.2f}")
                            # send_discord_message(f"{t} / price:{cur_price}/ 시가:{df_open_1} / ma20:{ma20:,.2f}")  
                            if value_2 < value_1 :                     #6.현재봉 거래량이 1봉전 거래량보다 큼 (상승지표) 
                                print(f"{t} / price:{cur_price}/ 시가:{df_open_1} / ma20:{ma20:,.2f}/value2:{value_2:,.0f} / value1:{value_1:,.0f}") 
                                send_discord_message(f"{t} / value2:{value_2:,.0f} / value1:{value_1:,.0f}")  
                            
                                if ai_decision == "BUY" :
                                    print(f"{t} / AI:{ai_decision}")
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                ...
                "content": [
                    {
                "type": "text",
                "text": "You are the ultimate expert on cryptocurrency investing. You have decades of know-how, especially in short-term trading, and a tremendous ability to generate profits."
                    },
                    {
                "type": "text",
                "text": "I will sell when the yield is above 0.7%. I want it to tell me when to buy at 1%+ profit based on a single trade."
                    },
                    {
                "type": "text",
                "text": "Moving Average (MA), Volatility, and Relative Strength Index (RSI) indicators based on the given chart data to make professional price forecasts, tell us whether you're currently buying, selling."
                    },
                     {
                "type": "text",
                "text": "In particular, I would like to use a professional price forecasting technique to predict the price. I would like to use Linear Regression, Random Forest, and I don't want the forecast to take too long and miss the right buy and sell points."
                    },
                    {
                "type": "text",
                "text": "Then give me the response in json format.\n\nResponse Example:\n{\"decision\": \"BUY\"}\n{\"decision\": \"SELL\"}\n{\"decision\": \"HOLD\"}"
                    }
                ]
                },
                {
...
def trade_buy(ticker, k):
    ...
    if buyed_amount == 0 and ticker.split("-")[1] not in ["BTC", "ETH"] and krw >= 5000 :  # 매수 조건 확인
        for attempt in range(max_retries):
            ...
            # If current price is not in range, wait before retrying
            time.sleep(5)
        # After 10 attempts, if price is still not in range
...
def trade_sell(ticker):
    ...
    sell_end = selltime.replace(hour=9, minute=10, second=50, microsecond=0)         #VC
    ...
    if sell_start <= selltime <= sell_end:      # 매도 제한시간이면
        ...
        if buyed_amount > 0 :  # 보유잔고가 0 이상이면
            if profit_rate > 0.65:  # 0.65% 이상 수익률일 때 AI의 판단을 구함
                ...
                if ai_decision == "SELL" or profit_rate > 2.0 :
...
while True:
    try:
        ...
        """매수제한시간 체크 : EC2 변환"""
        restricted_start = stopbuy_time.replace(hour=8, minute=30, second=0, microsecond=0)     # 08:00  #vC2
