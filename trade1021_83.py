    ...
    for t in tickers:
        ...
        try:
            ...
            if day_value_1 >= 25_000_000_000 : 
                    ...
                    # if day_close_price_1 <=cur_price and day_open_price_1*1.05 >= cur_price :                            
    ...
    if sell_start <= selltime <= sell_end:      # 매도 제한시간이면
        ...
        if profit_rate >= 0.80:  # 수익률이 0.8% 이상일 때
            while attempts < max_attempts:
                ...
                if profit_rate <= 0.6 or profit_rate >= 1.5:  # 수익률이 0.6% 이상이거나 1.5% 이상일 때
                    sell_order = upbit.sell_market_order(ticker, buyed_amount)
                    send_discord_message(f"매도: {ticker}/ 현재가 {current_price}/ 수익률 {profit_rate:.2f}%")
                    return sell_order
                else:
                    print("수익률이 0.6% 초과 1.5% 미만입니다. 계속 감시합니다.")
