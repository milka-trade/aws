...
def load_ohlcv(ticker):
    ...
    if ticker not in df_tickers:   # 티커가 캐시에 없으면 데이터 가져오기     
        try:
            df_tickers[ticker] = pyupbit.get_ohlcv(ticker, interval="minute60", count=60) 
...
def get_ma30(ticker):
    df = load_ohlcv(ticker)   # 데이터프레임을 한 번 로드하고 재사용
    return calculate_moving_average(df, 30)

def get_ma60(ticker):
    df = load_ohlcv(ticker)   # 데이터프레임을 한 번 로드하고 재사용
    return calculate_moving_average(df, 60)
...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            yesterday_open_price = df_day['open'].iloc[-1]   # 어제 시가 조회
            yesterday_close_price = df_day['close'].iloc[-1]   # 어제 종가 조회
            ...
            if current_price is not None and today_open_price is not None:  # 현재가와 시가가 모두 유효한 경우
                if yesterday_close_price > yesterday_open_price :   #어제 종가가 어제 시가보다 크면(양봉) 제외
                    if current_price > today_open_price * 1.05: # 일봉기준 시가 대비 5% 이상 상승한 경우 제외
                        continue
           
            if today_volume >= 20_000_000:  # 전일 거래량이 20백만 이상
...
def trade_sell(ticker):
    ...
    """매도제한시간 체크 : EC2 변환"""
    sell_start = now.replace(hour=23, minute=59, second=00, microsecond=0)    #EC2
    sell_end = now.replace(hour=23, minute=59, second=50, microsecond=0)      #EC2
        ...
        try:
            ...
            for b in balances:
                ...
                if buyed_amount > 0:
                    ai_decision = get_ai_decision(ticker)
                    profit_rate = (current_price - avg_buy_price) / avg_buy_price * 100 if avg_buy_price > 0 else 0  # 수익률 계산
                    report_message += f"{b['currency']} 평균가 {avg_buy_price:.3f} 현재가 {current_price} 수익률 {profit_rate:.1f}% AI {ai_decision} \n"
...
while True:
    try:
        ...
        # restricted_end = current_time.replace(hour=9, minute=20, second=0, microsecond=0)  # 09:30   #vC2
        restricted_start = now.replace(hour=23, minute=0, second=0, microsecond=0)  # 23:00  #EC2
        restricted_end = now.replace(hour=0, minute=20, second=0, microsecond=0)  # 00:30    #EC2
        ...
        if krw_balance < 10_000:       # 잔고가 매수설정금액 미만일 경우
            ...
            if restricted_start <= current_time <= restricted_end:
                restricted_time = now.strftime('%Y-%m-%d %H:%M:%S')  # 매수제한시간
                send_discord_message(f"매수금지 {restricted_time} ")
                time.sleep(180)  # 매수제한시간 3분 대기
                continue  
