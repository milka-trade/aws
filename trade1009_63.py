...
def send_discord_message(msg):
    ...
    try:
        ...
        # print(f"{err_time} 디스코드 메시지 전송 실패 : {e}")
...
def load_ohlcv(ticker):
    global df_tickers
    df_tickers.clear()  # # 데이터프레임 초기화: 모든 티커 초기화
    ...
    for t in tickers:
        ...
        try:
            ...
            if current_price is not None and today_open_price is not None:  # 현재가와 시가가 모두 유효한 경우에만 비교
                if current_price < today_open_price and current_price > today_open_price * 1.03: # 현재가가 시가 대비 5% 이상 상승한 경우 제외
                    continue
    ...
    if is_within_restricted_time():     #for EC2    /# Check if we are within the restricted time frame
        ...
        if buyed_amount == 0 and ticker.split("-")[1] not in ["BTC", "ETH"] and krw >= 5000 :  # 매수 조건 확인
            ...
            if target_price <= current_price and current_price < target_price*1.05 :  #현재가가 목표가 이상이면서 목표가의 5% 이내
                print(f"{try_time} 매수시도: {ticker}, 목표가: {target_price}, 현재가: {current_price}")    #이평5: {ma5:.2f}, 이평15: {ma15:.2f}, , AI: {ai_decision}
                ...
                if ai_decision != 'SELL' :  # AI의 판단이 NOT SELL이면
                    try:
                        ...
                        send_discord_message(f"매수: {buy_time}, {ticker}, 목표가: {target_price} 현재가: {current_price} 매수금액:{buy_amount} 이평5: {ma5:.2f}, 이평15: {ma15:.2f}, AI: {ai_decision}")
                        time.sleep(5)  # API 호출 제한을 위한 대기
...
def trade_sell(ticker):
    ...
    if current_time.hour == 23 and current_time.minute == 59 and 0 <= current_time.second < 50:
        ...
        for b in balances:
            if b['currency'] not in ["KRW", "AUCTION", "QI", "ONX", "ETHF", "ETHW", "PURSE"]:  # 보유 잔고가 있는 경우
                ...
                sell_time = current_time.strftime('%Y-%m-%d %H:%M:%S')  # 매도 시간 기록
                send_discord_message(f"전량 매도 시간: {sell_time}, Ticker: {ticker}, 수익률: {profit_rate:.2f}%")
                # client.chat_postMessage(channel='#api_test', text=str(f"전량 매도 시간: {sell_time}, Ticker: {ticker}, 수익률: {profit_rate:.2f}%")) 
                return sell_order
    
    if evaluation_amount > 5000 :  # 평가 금액이 5000 이상인 경우에만 매도 조건 체크
        if currency not in ["BTC", "ETH"]:
        
            if profit_rate > 0.6:  # 1.3% 이상 수익률일 때 AI의 판단을 구함
                ai_decision = get_ai_decision(ticker)  # AI의 판단을 구함
            
                if ai_decision != 'BUY' or profit_rate > 2:  # AI의 판단이 BUY가 아니거나 수익률이 5%를 넘는 경우 매도
                    ...
                    send_discord_message(f"매도 시간: {sell_time}, Ticker: {ticker}, 수익률: {profit_rate:.2f}% (수익률 5% 초과 또는 AI 판단 SELL)")
                    # client.chat_postMessage(channel='#api_test', text=str(f"매도: {sell_time}, Ticker: {ticker}, 수익률: {profit_rate:.2f}% (수익률 5% 초과 또는 AI 판단 SELL)")) 
                    return sell_order

                sell_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # 매도 시도 시간 기록
                # print(f"매도 시도 시간: {sell_time}, 코인명 : {currency}, AI의 판단: {ai_decision}, 수익률: {profit_rate:.2f}%: 매도 조건 미충족")
                send_discord_message(f"매도 시도 시간: {sell_time}, 코인명 : {currency}, AI의 판단: {ai_decision}, 수익률: {profit_rate:.2f}%: 매도 조건 미충족")


            # if profit_rate < -3:  # -3% 이하 손실 시 AI 판단 후 매도
            #     ai_decision = get_ai_decision(ticker)  # AI의 판단을 구함
            ...
            #         send_discord_message(f"매도 시간: {sell_time}, Ticker: {ticker}, 수익률: {profit_rate:.2f}% (-5% 이하)")
            #         return sell_order

            #     sell_time = datetime.now().strftime('%Y-%m-%d %H:%M:%S')  # 매도 시도 시간 기록
            #     # print(f"매도 시도 시간: {sell_time}, 코인명 : {currency}, AI의 판단: {ai_decision}, 수익률: {profit_rate:.2f}%: 매도 조건을 충족하지 않았습니다.")
            #     send_discord_message(f"매도 시도 시간: {sell_time}, 코인명 : {currency}, AI의 판단: {ai_decision}, 수익률: {profit_rate:.2f}%: 매도 조건을 충족하지 않았습니다.")
...
#             send_discord_message(f"Sold full balance at: {sell_time}, Ticker: {ticker}, price: {current_price}, 매도량: {buyed_amount}, Profit: {profit_rate:.2f}%, AI: {ai_decision}")
#             return sell_order
...
#                     send_discord_message(f"매도: {sell_time}, Ticker: {ticker}, 현재가:{current_price},매도량: {buyed_amount}, Profit: {profit_rate:.2f}%, AI: {ai_decision}")
#                     time.sleep(1)  # API 호출 제한을 위한 대기
#                     return sell_order
...
#                 # send_discord_message(f"매수 시도 : best_ticker {best_ticker}, interest: {interest:.2f}, best_k : {best_k:.2f}")
#                 result = trade_buy(best_ticker, best_k)  # 매수 실행
#                 time.sleep(360)  # 매수 후 잠시 대기
                
#                 result = sell_all_assets()  # 보유 자산 매도
...
while True:
    try:
        ...
        if krw_balance < 100000:
            ...
            # 비트코인 60분봉 데이터 조회
            df_btc = pyupbit.get_ohlcv("KRW-BTC", interval="minute60", count=2)   
            if df_btc is not None and not df_btc.empty:
                today_open_price = df_btc['open'].iloc[-1]  # 최근 60분봉의 시가
                current_price = df_btc['close'].iloc[-1]  # 최근 60분봉의 현재가

                # 양봉 조건 확인
                if current_price > today_open_price:  # 현재가가 시가보다 큰 경우
                    best_ticker, interest, best_k = get_best_ticker()  # 최고의 코인 조회
                    send_discord_message(f"비트코인 60분봉 양봉, 매수시작 : best_ticker {best_ticker}, best_k : {best_k}")
                    # client.chat_postMessage(channel='#api_test', text=str(f"비트코인 60분봉 양봉, 매수시작 : best_ticker {best_ticker}, best_k : {best_k}"))
                    
                    if best_ticker:  # 최고의 코인이 존재할 경우
