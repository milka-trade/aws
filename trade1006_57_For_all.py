...
def filtered_tickers(tickers, held_coins):
    ...
    for t in tickers:
        ...
        try:
            ...
            # 거래량 증가 비율 계산
            if today_volume >= 15_000_000:  # 현재봉 거래량이 15백만 이상
                df_open=df['open'].iloc[-1]
                if current_price >= df_open:  # 현재가 양봉
                    if current_price <= get_ma5(t)*1.2:  # 현재가 양봉
    ...
    if evaluation_amount > 5000:  # 평가 금액이 5000 이상인 경우에만 매도 조건 체크
        if ticker.split("-")[1] not in ["BTC", "ETH"]:
            if profit_rate > 0.6:  # 수익률 조건
                ...
                if ai_decision == 'SELL' or profit_rate > 1.0:  # AI의 판단이 NOT BUY이거나 수익률이 5.0%를 넘는 경우 매도
